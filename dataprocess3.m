...
sorted_angle = reshape(angle_profile,n_chirps*n_samples*Q,1);
sorted_angle = sort(sorted_angle,'descend');
num = 10;
max10 = sorted_angle(1:num);
row = zeros(num);
col = zeros(num);
pag = zeros(num);
for i = 1:num
    [row(i),col(i),pag(i)]=ind2sub(size(angle_profile),find(angle_profile==max10(i)));
    %% 计算目标距离、速度、角度
    fb = ((row(i)-1)*Fs)/n_samples;         %差拍频率
    fd = ((col(i)-n_chirps/2-1)*Fs)/(n_samples*n_chirps); %多普勒频率
    fw = (pag(i)-Q/2-1)/Q;          %空间频率
    R = c*(fb-fd)/2/Slope_Hz;          %距离公式
    v = lambda*fd/2;            %速度公式
    theta = asin(fw*lambda/d);  %角度公式
    angle = theta*180/pi;
    fprintf('目标%d距离： %f m\n',i,R);
    fprintf('目标%d速度： %f m/s\n',i,v);
    fprintf('目标%d角度： %f°\n',i,angle);
end
